...
def train(X, Y, Xtest, Ytest):
    learning_rate = 0.03
    #keep_prob =   [ 1.0,  0.80,  0.80,  0.85,  0.85, 0.85, 0.95, 0.95,    1] 
    #layers_list = [3072,   512,   512,   256,   256,  256,   64,   64,   10]
    ...
    print('backprop started')
    try:
        for info in model.momentum_train(iterations=10000, yld=10):        
            print(template.format(**info))
            # if np.isnan(info["train_cost"]) and  info["iteration"] >= 80:
            #     break
    finally:    
        weights = model.W, model.B        
